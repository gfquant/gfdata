...
class stock__get_locked_shares(BaseModel):
...
class stock__get_billboard_list(BaseModel):
    stock_list: str = CODE_SEP.join(['000001.XSHE', '600000.XSHG'])
    start_date: Optional[str] = None  # TRADING_PRE_DT
    end_date: Optional[str] = TRADING_TODAY
    count: int = None
...
class stock__get_money_flow(BaseModel):
...
class stock__get_concept(BaseModel):
...
class stock__get_concept_stocks(BaseModel):
    concept_code: str = 'GN036'
    date: Optional[datetime.date] = TRADING_TODAY


class stock__get_concepts(BaseModel):
    pass


class stock__get_industry(BaseModel):
    security: str = CODE_SEP.join(['000001.XSHE', '600000.XSHG'])
    date: Optional[datetime.date] = TRADING_TODAY


class stock__get_industry_stocks(BaseModel):
    industry_code: str = '801010'
    date: Optional[datetime.date] = TRADING_TODAY


class get_industries__name(Enum):  # get_extras函数的info参数
    zjw = 'zjw'
    sw_l1 = 'sw_l1'
    sw_l2 = 'sw_l2'
    sw_l3 = 'sw_l3'
...
class stock__get_industries(BaseModel):
    name: get_industries__name = 'zjw'
    date: Optional[datetime.date] = TRADING_TODAY


class stock__get_index_weights(BaseModel):
...
class stock__get_marginsec_stocks(BaseModel):
    date: Optional[datetime.date] = TRADING_TODAY


class stock__get_margincash_stocks(BaseModel):
    date: Optional[datetime.date] = TRADING_TODAY


class stock__get_index_stocks(BaseModel):
